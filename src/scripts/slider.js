...
for(let i = sliderImgs.length - 1; i >= 0; i--) {
    if(dir[dir.length - 1] !== "index.html") {
        ...
        sliderImgs[sliderImgs.length - i - 1].style.backgroundImage = `url(./src/images/slider-imgs/slider-img-${i}.png)`;
    }
}
...
for(let i = 0; i < 6; i++) {
    ...
    console.log(dir[dir.length - 1] !== "index.html");

    sliderImgs[i].addEventListener("mouseover", () => {
        if(dir[dir.length - 1] !== "index.html") {
            sliderImgs[i].style.backgroundImage = `url(../images/slider-imgs/slider-img-${i}.png)`;
        } else {
            sliderImgs[i].style.backgroundImage = `url(./src/images/slider-imgs/slider-img-${i}.png)`;
        }
    })
